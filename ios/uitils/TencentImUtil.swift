  ...
  public static func createMessage(param: Dictionary<String, Any>, resolve: @escaping RCTPromiseResolveBlock, type: Int) -> V2TIMMessage {
    ...
    switch type {
      ...
      case 2:
        ...
        break
      case 3:
        let imagePath = messageMap["imagePath"] as! String;
        let createdMessage = V2TIMManager.sharedInstance()?.createImageMessage(imagePath);
        message = createdMessage ?? V2TIMMessage();
        break
      case 4:
    ...
    * 返回[错误返回闭包]，腾讯云IM通用格式
    */
	public static func returnErrorClosures(method: String, resolve: @escaping RCTPromiseResolveBlock) -> V2TIMFail {
       return {
           (code: Int32, desc: Optional<String>) -> Void in
            CommonUtils.resultFailed(desc: desc, code: code, method: method, resolve: resolve)
       };
   }
