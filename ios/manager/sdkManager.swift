...
class SDKManager {
	...
	public func initSDK(param: [String: Any], resolve: @escaping RCTPromiseResolveBlock) {
		if let sdkAppID = param["sdkAppID"] as? Int32,
		   let logLevel = param["logLevel"] as? Int {
				V2TIMManager.sharedInstance().callExperimentalAPI("setUIPlatform",param: 18 as NSObject,succ:{_ in
	...
	*/
	...
	public func subscribeUserStatus(param: [String: Any], resolve: @escaping RCTPromiseResolveBlock) {
        let userIDList:[String] = param["userIDList"] as! [String];
        
        V2TIMManager.sharedInstance().subscribeUserStatus(userIDList) {
            CommonUtils.resultSuccess(method: "subscribeUserStatus", resolve: resolve)
        } fail: { code, desc in
            CommonUtils.resultFailed(desc: desc, code: code, method: "subscribeUserStatus", resolve: resolve)
        }
    }

	public func unsubscribeUserStatus(param: [String: Any], resolve: @escaping RCTPromiseResolveBlock) {
        let userIDList:[String] = param["userIDList"] as! [String];

        V2TIMManager.sharedInstance().unsubscribeUserStatus(userIDList) {
            CommonUtils.resultSuccess(method: "unsubscribeUserStatus", resolve: resolve)
        } fail: { code, desc in
            CommonUtils.resultFailed(desc: desc, code: code, method: "unsubscribeUserStatus", resolve: resolve)
        }
    }
}