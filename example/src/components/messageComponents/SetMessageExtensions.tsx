...
const SetMessageExtensionsComponent = () => {
    const [res, setRes] = useState<any>({});
    const [conversationID, setConversationID] = useState<string>('未选择');
    const [message, setMessage] = useState<string>('[]');

    const setMessageExtensions = async () => {
        const res = await TencentImSDKPlugin.v2TIMManager
            .getMessageManager()
            .setMessageExtensions(message, [
                { extensionKey: 'testKey1', extensionValue: 'testValue3' },
                { extensionKey: 'testKey2', extensionValue: 'testValue4' },
            ]);
...
export default SetMessageExtensionsComponent;
