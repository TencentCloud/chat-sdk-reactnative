...
const GetHistoryMessageListComponent = () => {
    ...
    const [communityGroupID, setCommunityGroupID] = useState<string>('');
    ...
    const getHistoryMessageList = async () => {
        ...
        if ((receiver && !groupID) || (!receiver && groupID)) {
            ...
            if (res.data!.length > 0) {
                setLastMessageID(
                    res.data![res.data!.length - 1].msgID!.toString()
                );
                setLastMessageSeq(
                    parseInt(res.data![res.data!.length - 1].seq!.toString())
                );
            } else {
                ...
                <View style={styles.userInputcontainer}>
                    <View style={mystylesheet.itemContainergray}>
                        <View style={styles.prioritySelectView}>
                            ...
                            <Text
                                style={styles.prioritySelectText}
                            >{`已选：${typeName}`}</Text>
                        </View>
                    </View>
                </View>
                <BottomModalComponent
                    type="historytypeselect"
                    visible={visible}
