...
const SearchGroupMembersComponent = () => {
    const searchGroupMembers = async () => {
        ...
        keys.forEach((key) => {
            const searchedMembers = responseData![key];
            searchedMembers.forEach((item) => {
    ...
    const searchMemberNickNametoggle = () =>
        setIsSearchMemberNickName((previousState) => !previousState);
    const searchMemberUserIDtoggle = () =>
        setIsSearchMemberUserID((previousState) => !previousState);
    const searchMemberNameCardtoggle = () =>
        setIsSearchMemberNameCard((previousState) => !previousState);
    const searchMemberRemarktoggle = () =>
        setIsSearchMemberRemark((previousState) => !previousState);
    const CodeComponent = () => {
    ...
    return (
        <View style={{ height: '100%' }}>
            <View style={mystylesheet.userInputcontainer}>
                <UserInputComponent
                    content="搜索关键词列表，最多支持5个"
                    placeholdercontent="关键词(example只有设置了一个关键词)"
                    getContent={setKeywords}
                />
            </View>
